...
parseInt(len[1],16)*2;if(lenBytes>1)for(var i=2;i<=lenBytes;i+=2)len+=sequence[i]+sequence[i+1];return len};this.getTotalLen=function(len){if(len.length>2){len=len.substring(2);return parseInt(len,16)*2}else return parseInt(len,16)*2};this.parseTLV=function(sequence){sequence=this.removeWhiteSpaces(sequence);while(sequence!==""){obj=this.getTLV(sequence);sequence=sequence.substring(obj.getTotalSequence());this.tlv.push(obj)}for(var i=0;i<this.tlv.length;i++)this.tlv[i].computeChildren()};this.getTLV=
function(sequence){tmp=new TLVObject(null,null,null);tag=this.getTag(sequence);sequence=sequence.substring(tag.length);len=this.getLen(sequence);sequence=sequence.substring(len.length);tmp.len=len;tmp.tag=tag;tmp.val=sequence.substring(0,this.getTotalLen(len));return tmp};this.removeWhiteSpaces=function(str){return str.replace(/ /g,"")}};