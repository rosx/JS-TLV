...
this.getTLV=function(sequence){tmp=new TLVObject(null,null,null);tag=this.getTag(sequence);sequence=sequence.substring(tag.length);len=this.getLen(sequence);sequence=sequence.substring(len.length);tmp.len=len;tmp.tag=tag;tmp.val=sequence.substring(0,this.getTotalLen(len));return tmp};this.removeWhiteSpaces=function(str){return str.replace(/ /g,"")};this.encode=function(tag,val){tag=this.removeWhiteSpaces(tag);val=this.removeWhiteSpaces(val);tmp=new TLVObject(tag,null,val);lengthVal=(val.length/2).toString(16);
if(lengthVal.length%2!=0)lengthVal="0"+lengthVal;if(parseInt(lengthVal,16)>128){len="8"+(lengthVal.length/2).toString(16);len+=lengthVal}else len=lengthVal;tmp.len=len;return tmp};this.encodeDGI=function(tag,val){tag=this.removeWhiteSpaces(tag);val=this.removeWhiteSpaces(val);tmp=new TLVObject(tag,null,val);lengthHexa=(val.length/2).toString(16);if(lengthHexa.length%2!==0)lengthHexa="0"+lengthHexa;len="";if(parseInt(lengthHexa,16)>254){len+="FF";len+=lengthHexa}else len=lengthHexa;tmp.len=len;return tmp}}
;